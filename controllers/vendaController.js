...
module.exports = {
  ...
  getVenda: async function (req, res, next) {
    let venda = await vendaModel.getById(req.params.id_venda);
    if (venda) res.status(200).json({ venda });
    else
  ...
  putVenda: async function (req, res, next) {
    ...
    //Arrumar resposta
    vendaModel
      .update(id_venda, aux.valor_venda)
  ...
  deleteVenda: async function (req, res, next) {
    try {
      const id_venda = req.params.id_admin;
